...
interface MenuManagerProps {
  initialProductos: any[]; // Datos iniciales que vienen desde el servidor (Server Component)
}
...
export default function MenuManager({ initialProductos }: MenuManagerProps) {
  ...
  // Lista de productos (se inicializa con lo que trajo el servidor)
  const [productos, setProductos] = useState<any[]>(initialProductos);
  
  // Controla qué producto se está activando/desactivando para mostrar spinner de carga
  ...
  useEffect(() => {
    async function init() {
        try {
            // Verificamos quién es el usuario actual
            const user: any = await getCurrentUser();
            
            // Regla de Negocio: Solo Admin (1) o Líder (5) pueden ver esto
            ...
            // Quitamos el spinner de carga inicial
            setLoadingSeguridad(false);
        }
    }
    init();

    // Mantenemos sincronizados los productos si la prop inicial cambia
  ...
  // Helper visual para asignar colores a las categorías (Badges)
  ...
  // Abre el modal. Si recibe un producto, es modo Edición. Si no, es Crear.
  ...
  const handleToggleEstado = async (id: number, estadoActual: boolean) => {
    setProcesandoAccion(id); // Activa spinner en el botón específico
    
    // Optimistic UI: Cambiamos visualmente antes de que el servidor responda
    ...
    try {
        ...
        if(res.success) {
            ...
            router.refresh(); // Recarga datos del servidor Next.js
        } else {
            throw new Error(res.message);
        }
    } catch (error: any) {
        toast.error("Error: " + error.message);
        setProductos(backup); // Revertimos si falló
    } finally {
  ...
  // ============================================================================
  // RENDERIZADO CONDICIONAL (Carga y Seguridad)
  // ============================================================================
  
  // 1. Cargando validación de usuario
  if (loadingSeguridad) {
      return (
  ...
  if (!autorizado) {
      return (
          <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 text-gray-600">
              ...
              <p className="mt-2 text-center px-4">Solo usuarios con perfil <b className="text-gray-800">Líder</b> o <b className="text-gray-800">Administrador</b> pueden gestionar el menú.</p>
          </div>
      );
  }
  ...
  return (
    <div className="min-h-screen bg-[#f8f9fa] pb-12 px-4 sm:px-8 pt-6">
      <div className="max-w-7xl mx-auto">
          ...
          {/* --- HEADER --- */}
          {/* [RESPONSIVE]: Flex col en móvil (apilado), row en PC (alineado) */}
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
            ...
            <button 
                onClick={() => abrirModal()} 
                className="w-full md:w-auto bg-[#ff6d22] hover:bg-[#e05e1b] text-white px-5 py-3 rounded-xl shadow-md transition-all flex items-center justify-center gap-2 font-bold text-sm transform active:scale-95"
            >
          ...
          {/* --- BARRA DE FILTROS --- */}
          {/* [RESPONSIVE]: Se ajusta de columna a fila según el tamaño de pantalla */}
          <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 mb-6 flex flex-col lg:flex-row gap-4">
              ...
              <div className="relative w-full lg:flex-grow">
                  ...
                  <input 
                    ...
                    className="w-full pl-11 pr-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-[#ff6d22] transition-all text-sm font-medium" 
                    value={busqueda} 
                    onChange={(e) => setBusqueda(e.target.value)} 
                  />
              </div>

              {/* Selectores: En móvil se apilan o ponen lado a lado si caben */}
              <div className="flex flex-col sm:flex-row gap-4 w-full lg:w-auto">
                 {/* Filtro Categoría */}
                 <div className="relative w-full sm:w-56">
                      <Utensils className="absolute left-3.5 top-3 w-4 h-4 text-gray-400" />
                      <select 
                        className="w-full pl-10 pr-8 py-2.5 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-[#ff6d22] appearance-none cursor-pointer text-sm font-medium text-gray-700" 
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
              ...
              <div className="overflow-x-auto">
                {/* [RESPONSIVE]: min-w-[800px] fuerza el ancho mínimo para que las columnas no se rompan */}
