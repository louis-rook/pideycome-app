...
import { getCurrentUser } from '@/lib/api/auth'; // Necesario para verificar rol
import { Plus, Edit, EyeOff, Image as ImageIcon, Search, Filter, Eye, Utensils, CheckCircle2, XCircle, ShieldAlert } from 'lucide-react';
import ProductModal from '@/components/menu/ProductModal';

export default function MenuGestionPage() {
  const [productos, setProductos] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);
  const [autorizado, setAutorizado] = useState(false); // Estado de seguridad
  ...
  useEffect(() => {
    const init = async () => {
        ...
        const rol = user?.Cargo?.toLowerCase() || "";
        
        // SOLO Lider y Admin
        if (rol.includes("admin") || rol.includes("lider")) {
            setAutorizado(true);
            await cargarDatos();
        } else {
            setAutorizado(false);
  ...
  const handleToggleEstado = async (id: number, estadoActual: boolean) => {
      ...
      const res = await toggleEstadoProducto(id, nuevoEstado); 
  ...
  if (loading) return <div className="min-h-screen pt-32 text-center text-gray-400 font-bold animate-pulse">Verificando permisos...</div>;
  ...
  return (
    <div className="min-h-screen bg-[#f8f9fa] pb-12 px-4 sm:px-8">
                    ...
                    <tbody className="divide-y divide-gray-50">
                        {productosFiltrados.map((prod) => (
                            <tr key={prod.ProductoID} className={`group transition-colors ${!prod.Activo ? 'bg-gray-50 opacity-60' : 'hover:bg-orange-50/20'}`}>
                                <td className="px-6 py-4">
                                    <div className="flex items-center gap-4">
                                        <div className="w-14 h-14 rounded-lg bg-gray-100 border border-gray-200 overflow-hidden shrink-0">
                                            {prod.Imagen ? <img src={prod.Imagen} alt={prod.Nombre} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center text-gray-300"><ImageIcon className="w-6 h-6" /></div>}
                                        </div>
                                        <div>
                                ...
                                <td className="px-6 py-4">
                                    <span className={`px-2.5 py-1 rounded-md text-[11px] font-bold border ${getCategoriaBadge(prod.NombreCategoria, prod.CategoriaID)}`}>{prod.NombreCategoria || 'General'}</span>
                                </td>
                                <td className="px-6 py-4">
                                    {prod.Activo ? (
                                ...
                                <td className="px-6 py-4 text-right">
                                    <div className="flex justify-end gap-2">
                                        <button onClick={() => abrirModal(prod)} className="p-2 bg-white border border-gray-200 text-gray-500 hover:text-[#ff6d22] hover:border-orange-200 rounded-lg transition-all shadow-sm">
                                            <Edit className="w-4 h-4" />
                                        </button>
                                        <button onClick={() => handleToggleEstado(prod.ProductoID, prod.Activo)} className={`p-2 bg-white border border-gray-200 rounded-lg transition-all shadow-sm ${prod.Activo ? 'text-gray-500 hover:text-red-600 hover:border-red-200' : 'text-gray-400 hover:text-green-600 hover:border-green-200'}`} title={prod.Activo ? "Desactivar" : "Activar"}>
      ...
      <ProductModal 
        ...
        onSuccess={() => { cargarDatos(); }} // Al guardar, recargamos la tabla
      />
    </div>
  );
}