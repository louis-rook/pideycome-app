...
import { Plus, Search, User, Users, Shield, Mail, Phone, CheckCircle2, XCircle, Eye, EyeOff, Edit, ShieldAlert } from 'lucide-react';
...
export default function PersonalPage() {
  ...
  useEffect(() => { 
    const init = async () => {
        ...
        const esAdmin = (user?.CargoID === 1) || (user?.Cargo?.toLowerCase().includes('admin'));
  ...
  const recargarDatos = async () => {
      const data = await getPersonasAdmin();
      setPersonas(data || []);
  };
  ...
  return (
    <div className="min-h-screen bg-[#f8f9fa] pb-12 px-4 sm:px-8">
                ...
                <table className="w-full text-left">
                    ...
                    <tbody className="divide-y divide-gray-50">
                        {personasFiltradas.map((p) => {
                            // SI ES EMPLEADO PERO ESTÁ INACTIVO, SE MUESTRA COMO CLIENTE
                            ...
                            <tr key={p.TerceroID} className={`group transition-colors ${!p.EmpleadoActivo && p.EsEmpleado ? 'bg-red-50/20' : 'hover:bg-orange-50/20'}`}>
                                ...
                                <td className="px-6 py-4">
                                    {/* VISUALIZACIÓN CORREGIDA: INACTIVO = CLIENTE */}
                                ...
                                <td className="px-6 py-4 text-right">
                                    <div className="flex justify-end gap-2">
                                        ...
                                        {p.EsEmpleado && (
                                            ...
                                            >
                                                {p.EmpleadoActivo ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
                                            </button>
                                        )}
                                    </div>
                                </td>
      ...
      <PersonaModal 
        ...
        listaCompleta={personas} // <--- PASAMOS LA LISTA PARA EL AUTOCOMPLETADO
